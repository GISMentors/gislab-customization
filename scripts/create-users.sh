...
tail -n +2 $1 | while read line || [ -n "$line" ];
do           
    fname=`echo $line | cut -d ',' -f3 | iconv -f utf-8 -t us-ascii//TRANSLIT`
    sname=`echo $line | cut -d ',' -f4 | iconv -f utf-8 -t us-ascii//TRANSLIT `
...
echo sudo gislab-adduser -g Lektor -l GISMentors -m gismentors@gislab.fsv.cvut.cz -p gismentors lektor
